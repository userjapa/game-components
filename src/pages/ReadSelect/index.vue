            ...
            <div class="item">
              ...
              <div class="container row line-bottom" v-for="(ex, index) in exercises">
                ...
                <div class="item col-5 no-padding-bottom">
                  <button class="btn btn-primary small" :disabled="$store.getters['read/isToUpdate']" @click="edit(ex, index)">Edit</button>
                  <button class="btn btn-danger small" :disabled="$store.getters['read/isToUpdate']" @click="$store.commit('read/removeExercise', index)">Remove</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="item flex-basis-400 item-border">
          <div class="container column">
            <div class="item">
              ...
              <div class="container">
                <div class="item">
                  <button class="btn btn-success" @click="save(exercise)" v-if="!$store.getters['read/isToUpdate']">Save</button>
                  <button class="btn btn-primary" @click="update(exercise)" v-if="$store.getters['read/isToUpdate']">Update</button>
                  <button class="btn btn-danger" @click="cancelUpdate()" v-if="$store.getters['read/isToUpdate']">Cancel</button>
                </div>
              </div>
            </div>
            <div class="item">
  ...
  methods: {
    save (exercise) {
      if (this.checkAnswers(exercise.answers)) {
        this.$store.commit('read/addExercise', exercise)
        this.exercise = {
          img: '',
          question: '',
          answered: false,
    ...
    edit (exercise, id) {
      this.$store.commit('read/setToUpdate', id)
      this.exercise = exercise
    },
    update (exercise) {
      this.$store.commit('read/update', exercise)
      this.cancelUpdate()
    },
    cancelUpdate () {
      this.exercise = {
      ...
      this.$store.commit('read/setToUpdate', null)
    }
  },
  computed: {
    exercises () {
      return this.$store.getters['read/getExercises']
    }
  },
  components: {
    ReadAnswers
