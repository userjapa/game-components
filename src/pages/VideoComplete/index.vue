    ...
    <div class="item margin-10 item-border flex-basis-300 flex-grow-1">
      <div class="">
        ...
        <form v-on:submit.prevent="addInterruption(interruption)">
          ...
          <div>
            <button class="btn btn-primary btn-full margin-bottom-10" type="submit" name="submit" v-if="!$store.getters['video/isToUpdate']">Add Interruption</button>
            <button class="btn btn-full margin-bottom-10" type="button" name="edit" v-if="$store.getters['video/isToUpdate']" @click="updateInterruption(interruption)">Edit Interruption</button>
            <button class="btn btn-full margin-bottom-10" type="button" name="cancel" v-if="$store.getters['video/isToUpdate']" @click="cancelUpdate()">Cancel Edit</button>
          </div>
        </form>
      </div>
    </div>
...
<script>
import VideoPlayer from '../../components/VideoGame'
import answerBus from '../../components/VideoAnswers/bus'
...
export default {
  ...
  methods: {
    addInterruption(interruption) {
      ...
      if (hasCorrectAnswer) {
        this.$store.commit('video/addInterruption', interruption)
        this.interruption = {
          message: '',
          pause: false,
          time: 0,
    ...
    editInterruption(index) {
      this.$store.commit('video/setToUpdate', index)
      this.interruption = Object.assign({}, this.video.interruptions[index])
    },
    updateInterruption(interruption) {
      this.$store.commit('video/edit', interruption)
      this.cancelUpdate()
    },
    cancelUpdate() {
      this.$store.commit('video/setToUpdate', null)
      this.interruption = {
        message: '',
        pause: false,
        time: 0,
    ...
    removeInterruption(index) {
      this.$store.commit('video/remove', index)
    },
    addAnswer(answer) {
  ...
  computed: {
    ...
    interruptions () {
      const interruptions = this.$store.getters['video/getVideoInterruptions']
